...
const createStore = (bindReducer, effects, options = {}, action$ = new Rx.Subject()) => {
  const reducers = [];
  const actions = {};

  bindEffects(action$, effects, actions);
...
export function bindEffects(action$, effects, actions) {
  ...
  Object.keys(effects).forEach((effectKey) => {
    ...
    if(isFunc(effect)) {
      return actions[effectKey] = bindEffect(action$, effect, actions);
    }

    return bindEffects(action$, effect, actions);
  });
}

export function bindEffect(action$, effect) {
  return (...args) => action$.next(effect(...args));
}
...
export function reducer(blueprint) {
  return (actions, action$) => {
    ...
    Object.keys(blueprint).forEach((key) => {
      actions[key] = bindEffect(action$, (payload) => {
        return {
          type: key,
          payload
        }
